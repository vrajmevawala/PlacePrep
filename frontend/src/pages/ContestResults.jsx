...
const ContestResults = () => {
  ...
  // If results exist, show them regardless of contest countdown/state.
  // Otherwise, show the "coming soon" screen (with countdown when contest info is available).
  const isContestEnded = contest ? new Date() > new Date(contest.endTime) : false;
  ...
  // Normalize time taken (backend returns minutes as number). If a string note exists, show that instead.
  const rawTimeTaken = results.timeTaken;
  const autoSubmitNote = results.autoSubmitNote;
  const timeTakenMinutes = typeof rawTimeTaken === 'number' && Number.isFinite(rawTimeTaken)
    ? rawTimeTaken
    : (typeof rawTimeTaken === 'string' ? parseInt(rawTimeTaken, 10) : 0);
  const timeTakenDisplay = autoSubmitNote
    ? autoSubmitNote
    : `${Math.floor(timeTakenMinutes / 60)}:${(timeTakenMinutes % 60).toString().padStart(2, '0')}`;
